...
function ancora_sc_accordion_item($atts, $content=null) {
	...
	$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
			...
			. ($title)
            . '<span class="sc_accordion_closed"></span>'
			. '</h5>'
...
/*
[trx_audio url="http://trex2.ancora.dnw/wp-content/uploads/2014/12/Dream-Music-Relax.mp3" image="http://trex2.ancora.dnw/wp-content/uploads/2014/10/post_audio.jpg" title="Insert Audio Title Here" author="Lily Hunter" controls="show" autoplay="off"]
*/
...
function ancora_sc_button($atts, $content=null){
	...
	$css .= ancora_get_css_position_from_values($top, $right, $bottom, $left, $width, $height)
        ...
        . 'border-style:solid;'
        . ($bd_color !== '' ? 'border-color:' . esc_attr($bd_color) .';' : '')
        . ($bd_size !== '' ? 'border-width: ' . esc_attr($bd_size) .'px;' : '');
...
function ancora_sc_chat($atts, $content=null){
	...
	if (!empty($photo)) {
		...
		$photo = ancora_get_resized_image_tag($photo, 52, 52);
	}
...
function ancora_sc_column_item($atts, $content=null) {
	...
	$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') . ' class="column-'.($span > 1 ? esc_attr($span) : 1).'_'.esc_attr($ANCORA_GLOBALS['sc_columns_count']).' sc_column_item sc_column_item_'.esc_attr($ANCORA_GLOBALS['sc_columns_counter'])
                . (!empty($is_hover) ? ' sc_columns_is_hover_' . esc_attr($is_hover) . ' ' : ' sc_columns_is_hover_no')
				. (!empty($class) ? ' '.esc_attr($class) : '')
...
function ancora_sc_contact_form($atts, $content = null) {
	...
	$output = '<div ' . ($id ? ' id="'.esc_attr($id).'"' : '') 
			...
			. (!ancora_sc_param_is_off($animation) ? ' data-animation="'.esc_attr(ancora_sc_get_animation_classes($animation)).'"' : '')
			. '>'
		. ($title ? '<h3 class="sc_contact_form_title">' . ($title) . '</h3>' : '')
		...
		. ($content != '' && ancora_sc_param_is_on($custom)
			...
			: '<div class="sc_contact_form_info sc_columns columns_wrap">'
                    .'<div class="column-1_2 sc_column_item sc_column_item_1">'
                        ...
                        .'<div class="sc_contact_form_item sc_contact_form_field label_over"><label class="required" for="sc_contact_form_message">' . __('Message', 'ancora') . '</label><textarea id="sc_contact_form_message" name="message" placeholder="' . __('Message', 'ancora') . '"></textarea></div>'
                    .'</div>'
                    .'<div class="column-1_2 sc_column_item sc_column_item_2">'
                        .'<div class="sc_contact_form_item sc_contact_form_field label_over"><label class="required" for="sc_contact_form_username">' . __('Last Name', 'ancora') . '</label><input id="sc_contact_form_lastname" type="text" name="lastname" placeholder="' . __('Last Name', 'ancora') . '"></div>'
                        .'<div class="sc_contact_form_item sc_contact_form_field label_over"><label class="required" for="sc_contact_form_phone">' . __('Phone', 'ancora') . '</label><input id="sc_contact_form_phone" type="text" name="phone" placeholder="' . __('Phone', 'ancora') . '"></div>'
                        .'<div class="sc_contact_form_item sc_contact_form_field label_over"><label class="required" for="sc_contact_form_service">' . __('Service', 'ancora') . '</label><input id="sc_contact_form_service" type="text" name="service" placeholder="' . __('I’m interested in service', 'ancora') . '"></div>'
                        .'<div class="sc_contact_form_item sc_contact_form_field"><button>'.__('Submit', 'ancora').'</button></div>'
                    .'</div>'
                .'</div>'
			)
		...
		if (!($contact_email = ancora_get_theme_option('contact_email')) && !($contact_email = ancora_get_theme_option('admin_email')))
			...
			if ($type=='contact') {
				...
				$msg = "\n".__('Name:', 'ancora')   .' '.esc_html($user_name)
                    .  "\n".__('Last Name:', 'ancora')   .' '.esc_html($last_name)
					.  "\n".__('E-mail:', 'ancora') .' '.esc_html($user_email)
                    .  "\n".__('Phone:', 'ancora') .' '.esc_html($user_phone)
					.  "\n".__('Message:', 'ancora').' '.esc_html($user_msg)
                    .  "\n".__('Service:', 'ancora').' '.esc_html($user_serv);
...
function ancora_sc_gap_wrapper($str) {
	...
	if ($str_new != $str) $str = preg_replace('/('.ancora_sc_gap_end().')[\r\n\s]*(<\/div>)[\r\n\s]*(<\/div>)[\r\n\s]*(<\/div>)/i', '\\2\\3\\4\\1', $str_new);
	// Gap layout
	return str_replace(
			array(
				ancora_sc_gap_start(),
...
function ancora_sc_google_map($atts, $content = null) {
	...
	ancora_enqueue_script( 'googlemap', 'http://maps.google.com/maps/api/js?sensor=false', array(), null, true );
...
function ancora_sc_icon($atts, $content=null){
	...
	$css2 = ($font_weight != '' && $font_weight != 'inherit' ? 'font-weight:'. esc_attr($font_weight).';' : '')
		...
		. ($color != '' ? 'color:'.esc_attr($color).';' : '')
		. ($bg_color != '' ? 'background-color:'.esc_attr($bg_color).';' : '')
        . ($bd_width != '' ? 'border-width: '.esc_attr($bd_width).'px;' : '')
        . ($bd_color != '' ? 'border-color:'.esc_attr($bd_color).';' : '')
	;
	$output = $icon!='' 
		? ($link ? '<a href="'.esc_url($link).'"' :(( esc_attr($icon_shadow) == 'yes') ? '<span class="sc_icon_shadow">' : '') . '<span') . ($id ? ' id="'.esc_attr($id).'"' : '')
			...
			.'>'
			.($link ? '</a>' : ((esc_attr($icon_shadow) == 'yes') ? '</span>' : '') . '</span>')
		: '';
...
function ancora_sc_image($atts, $content=null){
	...
	$output = empty($src) ? '' : ('<figure' . ($id ? ' id="'.esc_attr($id).'"' : '') 
		...
		. '>'
			...
			. (trim($title) || trim($icon) ? '<figcaption><span'.($icon ? ' class="'.esc_attr($icon).'"' : '').'></span><span class="figcaption_image_title"> ' . ($title) . '</span></figcaption>' : '')
		. '</figure>');
...
function ancora_sc_list($atts, $content=null){
    ...
    $ANCORA_GLOBALS['sc_list_icon'] = empty($icon) || ancora_sc_param_is_inherit($icon) ? "icon-right-2" : $icon;
...
function ancora_sc_parallax($atts, $content=null){
	...
	if ($bg_overlay > 0) {
		if ($bg_color='')
            {
                $bg_color = ancora_get_theme_bgcolor();
            }
	...
	$output = (ancora_sc_param_is_on($gap) ? ancora_sc_gap_start() : '')
		...
		. ($bg_overlay>0 || $bg_texture!=''
			? '<div class="sc_parallax_overlay'.($bg_texture>0 ? ' texture_bg_'.esc_attr($bg_texture) : '') . '"'
                . ($grad == 'no' ? (' style="' . ($bg_overlay>0 ? 'background-color:('.(int)$rgb['r'].','.(int)$rgb['g'].','.(int)$rgb['b'].','.min(1, max(0, $bg_overlay)).');' : '')) : (' style="background: -webkit-gradient(linear, left top, left bottom, color-stop(0%,#ffad26), color-stop(50%,#fea026), color-stop(51%,#ff8b00), color-stop(100%,#ff7701));background: -webkit-linear-gradient(top,  #ffad26 0%,#fea026 50%,#ff8b00 51%,#ff7701 100%);background: linear-gradient(to bottom, rgba(52, 184, 192, 0.9) 0%,rgba(111, 81, 199, 0.9) 100%);'))
//				. ' style="' . ($bg_overlay>0 ? 'background-color:('.(int)$rgb['r'].','.(int)$rgb['g'].','.(int)$rgb['b'].','.min(1, max(0, $bg_overlay)).');' : '')
//				. ' style="' . ($bg_overlay>0 ? 'background: linear-gradient(to bottom, rgba(52, 184, 192, 0.9) 0%,rgba(111, 81, 199, 0.9) 100%);' : '')
...
function ancora_sc_price($atts, $content=null){
	...
	if (!empty($money)) {
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
			...
			. '<span class="sc_price_money">'.($m[0]).'.'.($m[1]).'</span>'
...
function ancora_sc_price_block($atts, $content=null){
	...
	$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				...
				. (!ancora_sc_param_is_off($animation) ? ' data-animation="'.esc_attr(ancora_sc_get_animation_classes($animation)).'"' : '')
				. '>'
			. (!empty($title) ? '<div class="sc_price_block_title"'.(!empty($color) ? 'style="background:'.esc_attr($color).'; color:'.esc_attr($color).';' : '').'"><span style="color:#ffffff;">'.($title).'</span></div>' : '')
			...
			. (!empty($link_text) ? '<div class="sc_price_block_link">'.do_shortcode('[trx_button link="'.($link ? esc_url($link) : '#').'"'.(!empty($color) ? ' bg_color="'.esc_attr($color).'"' : '').' bd_color="#f9f9f9" bd_size="10"]'.($link_text).'[/trx_button]').'</div>' : '')
		. '</div>';
...
function ancora_sc_quote($atts, $content=null){
	...
	$content = do_shortcode($content);
    if (ancora_substr($content, 0, 2)!='<p') $content = '<p>' . ($content) . '</p>';
    if (esc_attr($type) == 'type_2')
    {
        if (empty($photo)) {
            if (!empty($email)){ $photo = get_avatar($email, 350*min(2, max(1, ancora_get_theme_option("retina_ready"))));} else {
                $photo = '<img class="sc_image sc_image_shape_round" width="80" height="80" src="http://1.gravatar.com/avatar/aecc247afe31f2b122cb7e115644abd6?s=80&amp;d=mm&amp;r=g">';
            }
        } else {
            if ($photo > 0) {
                $attach = wp_get_attachment_image_src( $photo, 'full' );
            ...
            $photo = ancora_get_resized_image_tag_for_team($photo, 80, 80);
        }
        $output = '<blockquote'
            . ($id ? ' id="' . esc_attr($id) . '"' : '') . ($cite_param)
            . ' class="sc_quote'
            . (!empty($class) ? ' ' . esc_attr($class) : '')
            . (!empty($type) ? ' ' . esc_attr($type) : '')
            . '"'
            . (!ancora_sc_param_is_off($animation) ? ' data-animation="' . esc_attr(ancora_sc_get_animation_classes($animation)) . '"' : '')
            . ($css != '' ? ' style="' . esc_attr($css) . '"' : '')
            . '>'
            . '<div class="sc_quote_avatar">'
                . ($photo)
            . '</div>'
				...
				. ' title="' . __('Open/close search form', 'ancora') . '">
					...
					<div class="search_form_wrap">
						...
						<div class="columns_wrap sc_columns">
                            <div class="clearfix column-1_2 sc_column_item sc_column_item_2 even"><input type="text" class="search_field" placeholder="' . esc_attr($title) . '" value="' . esc_attr(get_search_query()) . '" name="s" title="'.esc_attr($title).'" /></div>
							<div class="clearfix column-1_2 sc_column_item sc_column_item_1 odd first"><button type="submit" class="search_submit icon-zoom-1" data-text="Search" title="' . __('Start search', 'ancora') . '">Search</button></div>
                        </div>
						</form>
					</div>
				...
				. ($type=='pie' && ancora_sc_param_is_on($pie_compact)
					? ('<div class="sc_skills_legend">'.($title!='' ? '<h6 class="sc_skills_title">'.($title).'</h6>' : '').($ANCORA_GLOBALS['sc_skills_legend']).'</div>'
						. '<div id="'.esc_attr($id).'_pie" class="sc_skills_item">'
                            . '<span class="pie_diagram_max_value">'.$max_value.'%</span>'
...
function ancora_sc_skills_item($atts, $content=null) {
	...
	if ($ANCORA_GLOBALS['sc_skills_type'] == 'arc' || ($ANCORA_GLOBALS['sc_skills_type'] == 'pie' && ancora_sc_param_is_on($ANCORA_GLOBALS['sc_skills_pie_compact']))) {
		...
		$ANCORA_GLOBALS['sc_skills_legend'] .= '<div class="sc_skills_legend_item"><span class="sc_skills_legend_marker" style="background-color:'.esc_attr($color).';color:'.esc_attr($color).';"></span><span class="sc_skills_legend_title">' . ($title) . '</span><span class="sc_skills_legend_value">' . ($value) . '</span></div>';
		...
		if (in_array($ANCORA_GLOBALS['sc_skills_type'], array('bar', 'counter'))) {
			$output .= '<div class="sc_skills_count"' . ($ANCORA_GLOBALS['sc_skills_type']=='bar' && $color ? ' style="background-color:' . esc_attr($color) . '; border-color:' . esc_attr($color) . '"' : '') . '>'
...
function ancora_sc_tabs($atts, $content = null) {
	...
	for ($i = 0; $i < count($sc_tab_titles); $i++) {
        ...
        $icon = esc_attr($sc_tab_titles[$i]['icon']);
		$titles_output .= '<li class="'.join(' ', $classes).'">'
                            ...
                            . (!empty($icon) && !($icon == 'none') && !($icon == 'inherit') ? '<span class="sc_tabs_icon sc_icon sc_icon_shape_round '.$icon.'"></span>' : '')
                            . ($sc_tab_titles[$i]['title']) . '</a>'
							. '</li>';
	}
			...
			$output .= 	'<div class="column-1_'.esc_attr($columns) . '">'
							. '<div' . ($id ? ' id="'.esc_attr(($id).'_'.($post_number)).'"' : '') 
								...
								. '<div class="sc_team_item_info">'
									...
									. ($style==1 
										? '<div class="sc_team_item_description"><span class="sc_team_item_description_span">' . ($descr) . '</span><span class="sc_icon icon-plus sc_icon_shape_round"></span></div>' . ($socials)
										: '')
								. '</div>'
							. '</div>'
						. '</div>';
...
function cutStr($str, $length=40, $postfix='...')
{
...
function ancora_sc_team_item($atts, $content=null) {
	...
	if (empty($photo)) {
        ...
        $ANCORA_GLOBALS['sc_team_style']==1 ? $photo = ancora_get_resized_image_tag_for_team($photo, 80, 80) : $photo = ancora_get_resized_image_tag_for_team($photo, 120, 120);
	}
		...
		? '<div class="column-1_'.esc_attr($ANCORA_GLOBALS['sc_team_columns'])
				...
				. '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
					...
					. '<div class="sc_team_item_avatar">'
						. ($photo)
					. '</div>'
					. '<div class=team_item_info-container clearfix><div class="sc_team_item_info">'
						. '<h6 class="sc_team_item_title">' . ($link ? '<a href="'.esc_url($link).'">' : '') . ($name) . ($link ? '</a>' : '') . '</h6>'
						. '<div class="sc_team_item_position">' . ($position) . '</div>'
                        . '<div class="sc_team_item_description"><span class="sc_team_item_description_span">' . ($ANCORA_GLOBALS['sc_team_style'] == 1 ? cutStr($descr) : cutStr($descr)) . '</span>'
                                . ($ANCORA_GLOBALS['sc_team_style'] == 1 ? '<span class="sc_team_item_description_span_icon sc_icon icon-plus sc_icon_shape_round"></span>' : ' ')
                        . '</div>' . ($socials)
                            . ($ANCORA_GLOBALS['sc_team_style'] == 2 ? '<div class="sc_team_item_description_span_icon_container">'. ($link ? '<a href="'.esc_url($link).'">' : '') . '<span class="sc_team_item_description_span_icon sc_icon icon-plus sc_icon_shape_round"></span>' . ($link ? '</a>' : '') . '</div>' : ' ')
					. '</div></div>'
				. '</div>'
			. '</div>'
		: '';
