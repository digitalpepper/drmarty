...
function ancora_shortcodes_show_field(field, tab_idx) {
	...
	for (var clone_num in field['params']) {
		...
		for (var param_num in field['params'][clone_num]) {
			...
			case 'tags':
				...
				if (tags.length > 0) {
					for (i=0; i<tags.length; i++) {
						...
						output += '<span class="ancora_options_tag iconadmin-cancel">' + tags[i] + '</span>';
					}
				}
			...
			case "color":
				output += '<input class="ancora_options_input ancora_options_input_color'+(ANCORA_GLOBALS['shortcodes_cp']=='internal' || (ancora_isset(param['style']) && param['style']=='custom') ? ' ancora_options_input_color_custom' : '')+'"'
					+ ' name="' + id + '"'
					+ ' id="' + id + '"'
					+ ' type="text"'
					+ ' value="' + ancora_shortcodes_prepare_value(param['value']) + '"'
					...
					+ (!ancora_empty(param['action']) ? ' onchange="ancora_options_action_'+param['action']+'(this);return false;"' : '')
					+ ' />'
					+ (ANCORA_GLOBALS['shortcodes_cp']=='internal' || (ancora_isset(param['style']) && param['style']=='custom') ? '<span class="iColorPicker"></span>' : '');
			break;   
