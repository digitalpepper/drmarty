...
if (!$post_number) { 
	...
	if ($show_filters == 'categories') {			// Use categories as filter items
		...
		$portfolio_parent = $cur_term ? max(0, ancora_get_parent_taxonomy_by_property($cur_term->term_id, 'show_filters', 'yes', true, $taxonomy)) : 0;
		$args2 = array(
		...
		if (count($portfolio_list) > 0) {
			$filters .= '<a href="#" data-filter="*" class="'.esc_attr($filter_button_classes . ($portfolio_parent==$cur_term->term_id ? ' active' : '')) . '">' . __('All', 'ancora').'</a>';
...
if ($post_number > 0) {
	// Pagination
	$pagination = ancora_get_custom_option('blog_pagination');
